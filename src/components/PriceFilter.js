...
export default function PriceFilter({prop, obj, handlerUpdate, list}) {
	...
	return (
		<fieldset className={curClass}>
			...
			<div className="PriceFilter__list">
				<DropDownList type={prop + ".min"} title={data.titleMin} defaultValue={obj.min} update={handlerUpdate} list={minList} />
				<DropDownList type={`${prop}.max`} title={data.titleMax} defaultValue={obj.max} update={handlerUpdate} list={maxList} />
			</div>
		</fieldset>
	)
}